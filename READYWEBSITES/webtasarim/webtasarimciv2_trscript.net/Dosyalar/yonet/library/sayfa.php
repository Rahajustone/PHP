...
class kgPager { 
    ...
    function page_links ($inactive_page_tag, $pager_url_last) { 
        ...
        for ($i = $loop_start; $i <= $loop_finish; $i++) { 
			...
			$this -> page_links .= '<a '.$inactive_page_tag.'><span>'.$i.'</span></a>'; 
            }else{ 
                $this -> page_links .= '<a href="'.$this -> url.$i.$pager_url_last.'"><span>'.$i.'</span></a>'; 
            } 
        }
    } 
    ...
    function previous_page ($previous_page_text, $pager_url_last) { 
        if ($this -> current_page > 1) { 
            $this -> previous_page = '<a href="'.$this -> url.($this -> current_page - 1).$pager_url_last.'"  class="previous"><span>Previous</span></a>'; 
        } 
    } 

    function next_page ($next_page_text, $pager_url_last) { 
        if ($this -> current_page < $this -> total_pages) { 
            $this -> next_page = '<a href="'.$this -> url.($this -> current_page + 1).$pager_url_last.'"  class="next"><span>Next</span></a>'; 
        } 
    } 

    function first_page ($first_page_text, $pager_url_last) { 
        if ($this -> current_page > 1) { 
            $this -> first_page = '<a href="'.$this -> url.'1'.$pager_url_last.'" class="first"><span>First</span></a>'; // :) 
        } 
    } 

    function last_page ($last_page_text, $pager_url_last) { 
        if ($this -> current_page < $this -> total_pages) { 
            $this -> last_page = '<a href="'.$this -> url.$this -> total_pages.$pager_url_last.'" class="last"><span>Last</span></a>'; 
        } 
    } 
