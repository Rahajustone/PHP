...
if($_POST['rec']==1)
{
	$guncelle=mysql_query("UPDATE icerik SET baslik_tr='".$baslik."',baslik_eng='".$baslik2."',metin_tr='".$metin."',metin_eng='".$metin2."' WHERE id=".$sayfa." LIMIT 1");
	header("Location: index.php?page=iceriks&kat=".$kat."&sayfa=".$sayfa."");
}
if($_POST['foto_gun']==1)
{
	...
	{	
		...
		$resim_guncelle=mysql_query("UPDATE icerik SET resim='".$sonuc[0][1]."' WHERE id=".$sayfa." LIMIT 1");
	}

}
########################################################################################	
$icerik_sorgu=mysql_query("SELECT * FROM icerik WHERE id=".$sayfa." LIMIT 1");
    	...
    	<div class="box-body clear">
          ...
          <tr>
            ...
            <select name="kat" id="kat"  onchange="MM_jumpMenu('self',this,0)">
              ...
              <?php
			  $icerik_menu=mysql_query("SELECT * FROM icerik WHERE kat=".$kat." ORDER BY id ASC");
			  while($iyaz=mysql_fetch_assoc($icerik_menu))
			  {
				if($sayfa==$iyaz['id']) { $s=' selected="selected"'; } else { $s=''; }
				echo '<option value="index.php?page=iceriks&kat='.$kat.'&sayfa='.$iyaz['id'].'"'.$s.'>'.$iyaz['baslik_tr'].'</option>'; 
			  }
			  ?>
            </select></td>
          </tr>
