...
if($_POST['rec']==1)
{
	...
	if(!empty($_FILES['myfile']['name']))
	{
		...
		$resim_guncelle=mysql_query("UPDATE hizmet_alanlari SET resim='".$sonuc[0][1]."' WHERE id=".$id." LIMIT 1");	
	}
	
	$kayit=mysql_query("UPDATE hizmet_alanlari SET baslik_tr='".$baslik."',baslik_eng='".$baslik2."',metin_tr='".$metin."',metin_eng='".$metin2."' WHERE id=".$id." LIMIT 1");	
	if($kayit) { echo $islem->islem_sonucu(1,'İçerik Başarıyla Güncellenmiştir.'); }
}
$urun_sorgu=mysql_query("SELECT * FROM hizmet_alanlari WHERE id=".$id." LIMIT 1");
