...
{
	$sorgu=mysql_query("SELECT * FROM icerik WHERE kat=2 ORDER BY id ASC LIMIT 1");
	$yaz=mysql_fetch_assoc($sorgu);
	$id=$yaz['id'];
}
else
{
	$sorgu=mysql_query("SELECT * FROM icerik WHERE kat=2 AND id=".$id." LIMIT 1");
	$yaz=mysql_fetch_assoc($sorgu);
	$sec->zero($yaz['id']);
}
...
<div id="main">
    <div id="left">
        <div class="baslik"><?php echo MENU2; ?></div>
            <ul>
				<?php $fonks->content_menu('icerik',2,$id,'hizmet'); ?>
            </ul>
        <div class="clear"></div>
        <div id="bulten">
