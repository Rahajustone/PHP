...
{
	$kategori_sorgu=mysql_query("SELECT id,baslik_".$fonks->dil()." FROM kategori ORDER BY sira ASC LIMIT 1");
...
{
	$kategori_sorgu=mysql_query("SELECT id,baslik_".$fonks->dil()." FROM kategori WHERE id=".$id." LIMIT 1");
	$kategori=mysql_fetch_assoc($kategori_sorgu);
	$id=$kategori['id'];
	$baslik=$kategori['baslik_'.$fonks->dil()];		
}
...
$sorgu="SELECT id,baslik_".$fonks->dil().",resim FROM urunler WHERE kat=".$id." ORDER BY baslik_".$fonks->dil()." ASC ";
...
$pager_url = "urunler/"; // sayfalamanin yapildigi adres 
